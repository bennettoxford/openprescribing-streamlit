...
def get_measure_list():
    try:
        ...
        core_df = tags_df[
            ["table_id", "name", "tags", "radar_exclude", "is_percentage", "y_label"]
        ].copy()

        measure_list = core_df[
            (
                core_df["tags"].str.contains("core")
                | core_df["tags"].str.contains("lowpriority")
            )
        ...
        if MEASURE_LIST_CACHE.exists():
            st.warning(
                "Could not fetch latest measure list from GitHub, using cached version."
            )
...
@st.cache_data
def load_sicbl_df():
    return pd.read_csv(app_path / "csvs" / "sicbl.csv")[["code", "name"]].rename(
        columns={"name": "sicbl_name"}
    )


def find_improving_orgs(
    df, start_threshold=0.8, end_threshold=0.5, min_numerator=50, top_x=5
):
    """
            ...
            if start_mean > start_threshold:
                ...
                for j in range(i + 6, n - 5):
                    ...
                    if end_mean < end_threshold:
                        ...
                        rate_change = (
                            rates[i : i + 6].mean() - rates[j : j + 6].mean()
                        ) / rates[i : i + 6].mean()
                        if rate_change >= 0.05:
                            results.append(
                                {
    ...
    return (
        results_df.sort_values("end_percentile", ascending=False)
        .groupby("measure")
        .head(top_x)
    )
...
ir = ir.merge(
    measures[["table_id", "name", "is_percentage", "y_label"]].drop_duplicates(
        "table_id"
    ),
    left_on="measure",
    right_on="table_id",
...
org_df = ir_raw[
    (ir_raw["measure"] == selected_table_id) & (ir_raw["code"].isin(top_orgs))
...
table_df = (
    ir[ir["measure"] == selected_table_id][
        ["code", "start_percentile", "change_detected"]
    ]
