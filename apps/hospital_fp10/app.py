...
import duckdb
import time
from hospital_orgs import import_ord_data
...
def load_sql(filename: str) -> str:
    with open(Path(app_path) / "queries" / filename) as f:
        return f.read()
...
with st.sidebar:
    ...
    earliest_month = pd.to_datetime(
        query("SELECT MIN(CAST(date AS DATE)) FROM hospital_prescribing").iat[0, 0]
    )
...
with st.sidebar:
    ...
    FROM hospital_prescribing
    """)

    min_date = date_df.iloc[0, 0].date()
    max_date = date_df.iloc[0, 1].date()
...
predecessors = df.loc[
    df["ods_code"].isin(ods_codes) & df["closed_date"].notna()
]
...
if not predecessors.empty and (sel_prs or sel_icbs or sel_regions):
    ...
    for row in predecessors.itertuples(index=False):
        closed = pd.to_datetime(row.closed_date)
        parts.append(f"- {row.ods_name} (closed: {closed.day} {closed:%B %Y})")
...
with st.spinner("Loading table data..."):
    sql_in = "(" + ",".join(f"'{c}'" for c in ods_codes) + ")"

    detail_data = query(
        f"""
        SELECT bnf_name, rx.hospital, cd_category, bnf_chapter, sum(actual_cost) AS actual_cost, sum(items) AS items
        FROM {tool_name}_hospital_fp10 AS rx
        WHERE rx.hospital IN {sql_in}
        AND CAST(month AS DATE) BETWEEN DATE '{start_date}' AND DATE '{end_date}'
